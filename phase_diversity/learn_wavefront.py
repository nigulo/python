...
def train(model, Ds_train, coefs_train, Ds_test, coefs_test):
    #print("Training")
...
def test(model):
        ...
        Ds, DFs, coefs = gen_data(num_frames=n_test_data)
...
def gen_data(num_frames, num_images = None):
    image_file = 'icont'
    dir = "images"
    images, _, nx, nx_orig = utils.read_images(dir, image_file, is_planet = False)
...
if data is None:
    ...
    Ds, DFs, coefs = gen_data(num_frames)
...
for rep in np.arange(0, num_reps):
    ...
    if rep == num_reps-1:
        ...
        train(model, Ds, coefs, np.array([]), np.array([]), np.array([]))
    else:
        train(model, Ds_train, coefs_train, Ds_test, coefs_test)
