...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Ds, objs, diversity, positions, coords, coefs = self.datasets[data_index]
        ...
        coefs_out = np.array(coefs[obj_index, frame_index]).astype('float32')
        ...
        return Ds_out, diversities_out, coefs_out
      

    def get_data(self):
        Ds0, div0, coefs0 = self[0]
        ...
        coefs = np.empty((self.length(), coefs0.shape[0]))
        for i in range(self.length()):
            Ds_i, diversity_i, coefs_i = self[i]
            Ds[i] = Ds_i
            diversities[i] = diversity_i
            coefs[i] = coefs_i
        return Ds, diversities, coefs
...
class NN(nn.Module):
    ...
    def forward(self, data):
        ...
        image_input, diversity_input, true_alphas = data
        ...
        loss = torch.mean((true_alphas - alphas)**2)
        _, num, den, num_conj, psf, wf, DD = self.psf.mfbd_loss(image_input, alphas, diversity_input)
    ...
    def do_batch(self, Ds, diversity, true_alphas, train=True):
        Ds = Ds.to(device)
        diversity = diversity.to(device)
        true_alphas = true_alphas.to(device)
        if train:
            self.optimizer.zero_grad()
            result = self((Ds, diversity, true_alphas))
        else:
            with torch.no_grad():
                result = self((Ds, diversity, true_alphas))
        return result
    ...
    def do_epoch(self, data_loader, train=True, use_prefix=True, normalize=True):
        ...
        for batch_idx, (Ds, diversity, true_alphas) in enumerate(progress_bar):
            if normalize:
            ...
            #        my_test_plot.close()
            ###################################################################
            result = self.do_batch(Ds, diversity, true_alphas, train)
            if nn_mode == 1:
    ...
    def do_train(self):
        ...
        while len(obj_ids_used) < n_test and i < self.Ds_validation.length():
            ...
            obj_index, obj, _, _, _ = self.Ds_validation.get_obj_data(i)#np.reshape(self.objs[i], (self.nx, self.nx))
            found = False
    ...
    def do_test(self, dataset, file_prefix):
        ...
        _, _, _, coord, _ = Ds_test.get_obj_data(0)
        if coord is None:
        ...
        for i in range(Ds_test.length()):
            ...
            #    break
            obj_index_i, obj, _, _, true_alphas = Ds_test.get_obj_data(i)
            ...
            if benchmarking_level >= 2:
                obj_reconstr_true, psf_true, wf_true, loss_true = self.deconvolve(Ds_[:nf], true_alphas[:nf], diversity)
...
if train:
    ...
    model.do_test((Ds, objs, diversity, positions, coords, true_coefs/utils.mode_scale), "test")
