        ...
        with tf.device(gpu_id):
            ...
            if model is None:
                ...
                if no_shuffle:
                    ...
                    #alphas_layer2 = tf.slice(alphas_layer, [0, batch_size_per_gpu, 0], [1, batch_size_per_gpu, 512])
            ...
            if true_coefs is not None:
                ...
                for j in np.arange(nf):
                    if j % 100 == 0:
                        ...
                        my_test_plot.colormap(np.log(psf_true[j]+np.min(psf_true[j])+1), [0], show_colorbar=True)
                        my_test_plot.colormap(np.log(psf[j]+np.min(psf[j])+1), [1], show_colorbar=True)
