...
model = pickle.load(open('model_test_wo_length_scale.pkl', 'rb'))
...
for experiment_index in np.arange(0, num_experiments):
    ...
    opt_freq_label = freq_kmeans.predict([freq_gp])
    ...
    gpr_gp = GPR_QP.GPR_QP(sig_var=sig_var_gp, length_scale=1.0/freq_gp, freq=freq_gp, noise_var=noise_var,rot_freq=0.0, rot_amplitude=0.0, trend_var=trend_var_gp, c=0.0)
    ...
    with FileLock("GPRLock"):
        with open("test/results.txt", "a") as output:
            output.write("%s %s %s %s %s %s %s %s %s %s %s %s\n" % (index, f, f_opt_ls, f_opt_bglst, freq_gp, freq_se, length_scale, sig_var, sig_var_gp, trend_var, trend_var_gp, duration))  
