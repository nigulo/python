...
transformed parameters {
    ...
    vector[N] mu;
    #inv_length_scale = inv(length_scale*length_scale);
    for (i in 1:(N)) mu[i] = m;
}
...
model {
    ...
    for (i in 1:(N-1)) {
        for (j in (i+1):N) {
            Sigma[i, j] = sig_var * exp(-0.5 * freq * freq * pow(x[i] - x[j],2)) * cos(2*pi()*freq*(x[i] - x[j])) + trend_var * x[i] * x[j];
            Sigma[j, i] = Sigma[i, j];
        }
    }
    ...
    #length_scale ~ gamma(4.0/(freq+ 1.0/duration), 2);
