...
if os.path.isfile("GPR_stan_stars.txt"):
    #stars_to_recalculate = pd.read_csv("GPR_stan_stars.txt", names=['star'], dtype=None, sep='\s+', engine='python').as_matrix()
    stars_to_recalculate = data = np.genfromtxt("stars_to_recalculate.txt", dtype=None, delimiter=' ')
...
for root, dirs, dir_files in os.walk(data_dir):
    for file in dir_files:
        if file[-4:] == ".dat":
            ...
            if len(star_indices) > 0 or (not os.path.isfile("results/" + star + "_results.txt") and not os.path.isfile("results/" + star + "_0_results.txt")):
                files.append(file)
...
for i in np.arange(0, len(files)):
    ...
    for downsample_iter in np.arange(0, downsample_iters):
        ...
        gpr_gp = GPR_per.GPR_per(sig_var=sig_var, length_scale=length_scale, freq=freq, noise_var=noise_var_prop*noise_var, rot_freq=rot_freq, rot_amplitude=rot_amplitude, trend_var=trend_var, c=0.0)
        ...
        if rot_freq > 0:
            ...
            gpr_gp_wo_period = GPR_per.GPR_per(sig_var=sig_var, length_scale=length_scale, freq=freq, noise_var=noise_var_prop*noise_var, rot_freq=0.0, rot_amplitude=0.0, trend_var=trend_var, c=0.0)
        ...
        gpr_gp_null = GPR_per.GPR_per(sig_var=0.0, length_scale=0.0, freq=0.0, noise_var=noise_var_prop*noise_var, rot_freq=0.0, rot_amplitude=0.0, trend_var=trend_var, c=0.0)
        ...
        gpr_gp_seasons = GPR_per.GPR_per(sig_var=sig_var, length_scale=length_scale, freq=freq, noise_var=seasonal_noise_var*noise_var, rot_freq=0.0, rot_amplitude=0.0, trend_var=trend_var, c=0.0)
        ...
        gpr_gp_seasons_null = GPR_per.GPR_per(sig_var=0.0, length_scale=length_scale, freq=0.0, noise_var=seasonal_noise_var*noise_var, rot_freq=0.0, rot_amplitude=0.0, trend_var=trend_var, c=0.0)
