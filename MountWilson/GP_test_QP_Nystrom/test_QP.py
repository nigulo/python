...
for experiment_index in np.arange(0, num_experiments):
    ...
    fit = model.sampling(data=dict(x=t,N=n, M=len(seasonal_means),y=y,noise_var=noise_var_prop, var_y=np.var(y), var_seasonal_means=np.var(seasonal_means), freq_mu=prior_freq_mean, freq_sigma=prior_freq_std), 
