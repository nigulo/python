...
sys.path.append("..")
import numpy as np
#import numpy.random.poisson as poisson
...
if __name__ == '__main__':
    ...
    if probs is None:
        ...
        for s in States():
            for a in get_actions(s):
                probs[(s, a)] = get_probs(s, a)
        save(probs)

    result = load("result")
    if result is None:
        print("Optimizing...")
        dp = DP(States(), get_actions, probs)
        result = dp.solve()
        save(result, "result")
    v, q, pi = result
    ...
    print(v[(MIN_CARS, MIN_CARS, NIGHT)], v[(MAX_CARS, MAX_CARS, NIGHT)], v[(MIN_CARS, MAX_CARS, NIGHT)], v[(MAX_CARS, MIN_CARS, NIGHT)])
    print(pi[(MIN_CARS, MIN_CARS, NIGHT)], pi[(MAX_CARS, MAX_CARS, NIGHT)])
    print("Done.")
